...
Dashboard는 **사실 기반 요약 화면**이다. 사용자가 지금 어떤 음악을 듣고 있는지, 최근/중기 취향이 무엇인지, 최근 어떤 앨범과 아티스트를 중심으로 듣는지 한눈에 보여준다.
...
- 사용자가 최근에 어떤 앨범을 중심으로 듣고 있는지 상위 5개와 비율로 보여줌
- 장르 메타데이터 없이도 Dashboard에서 분포형 섹션을 유지함
...
- Spotify Recently Played가 제공하는 최신 최대 50건
...
1. Recently Played 응답에서 각 트랙의 `album.id`를 기준으로 묶는다.
2. 앨범별 등장 횟수를 카운트한다.
3. 전체 최근 재생 수 대비 각 앨범 비율을 계산한다.
4. 비율이 높은 순으로 정렬한다.
5. 상위 5개 앨범만 노출한다.
6. 같은 섹션에서 `고유 앨범 수(unique albums)`를 함께 표시한다.

**비율 공식**

- `albumRatio = 특정 앨범 count / 전체 recent track count`

**UI 예시**

- 앨범 커버 + 앨범명 + 비율 bar/list
- 상위 5개 앨범 비율
- 보조 문구: `고유 앨범 수 14개`
...
- 최근 재생 기준으로 특정 앨범에 얼마나 몰입해서 듣는지 보여줌
...
1. 최근 재생 데이터에서 앨범별 등장 횟수를 센다.
2. 가장 많이 등장한 앨범의 등장 횟수를 구한다.
3. `최다 등장 횟수 / 전체 최근 재생 수`를 계산한다.

**공식**

- `albumLoyalty = maxAlbumPlayCount / totalRecentTracks`
...
- 높을수록 최근에 한 앨범을 반복해서 듣는 경향이 강함

---

## 5.10 Album Diversity

**목적**

- 최근 재생 기준으로 얼마나 다양한 앨범을 오가며 듣는지 보여줌
...
1. 최근 재생 50건에서 앨범 id를 수집한다.
2. 중복 제거한 고유 앨범 수를 센다.
3. `고유 앨범 수 / 전체 최근 재생 수`로 비율 계산한다.

**공식**

- `albumDiversity = uniqueAlbumCount / totalRecentTracks`

**예시 해석**

- 높을수록 여러 앨범을 넓게 탐색하는 타입
- 낮을수록 특정 앨범 중심으로 반복 청취하는 타입
